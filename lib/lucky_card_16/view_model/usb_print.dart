...
class UsbPrintViewModel extends ChangeNotifier {
  ...
  Future<void> createAndPrintTicket(
    ...
    bytes += _buildReceiptRow(generator, 'Game Name', 'Lucky 16 Game');
    ...
    bytes += generator.hr();
    final bets = l16c.addLucky16BetsPrinting;
    for (int i = 0; i < (bets.length / 2).ceil(); i++) {
      int firstIndex = i * 2;
      int secondIndex = firstIndex + 1;
      final first = bets[firstIndex];
      Map<String, dynamic>? second;
      if (secondIndex < bets.length) {
        second = bets[secondIndex];
      }
      bytes += generator.row([
        PosColumn(
          text: getCardLabel(int.parse(first["game_id"].toString()), context),
          width: 3,
          styles: PosStyles(bold: true, align: PosAlign.center),
        ),
        PosColumn(
          text: first["amount"].toString(),
          width: 3,
        ...
        PosColumn(
          ...
          second != null
              ? getCardLabel(
            int.parse(second["game_id"].toString()),
            context,
          )
              : '',
          width: 3,
        ...
        PosColumn(
          text: second != null ? second["amount"].toString() : '',
          width: 3,
          styles: PosStyles(align: PosAlign.center),
        ),
      ]);
  ...
  String getCardLabel(int gameId, BuildContext context) {
    final cardList =
        Provider.of<Lucky16Controller>(context, listen: false).cardList;
    final card = cardList.where((e) => e.id == gameId).firstOrNull;
